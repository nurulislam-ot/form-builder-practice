...
import { COMPONENT, SIDEBAR_ITEM, ROW, COLUMN } from '../utils/constant'
import { ILayout } from '../utils/initial-data'

const ACCEPTS = [SIDEBAR_ITEM, COMPONENT, ROW, COLUMN]

interface Props {
  data: { childrenCount: number; path: string }
  onDrop: (data: { childrenCount: number; path: string }, item: string) => void
  isLast?: boolean
  className: string
}

const DropZone: React.FC<Props> = ({ data, onDrop, isLast, className }) => {
